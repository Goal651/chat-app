        ...
        return filteredGroups && filteredGroups.length > 0 ? filteredGroups.map(group => {
            return (
                <div onClick={() => chatNow(group)}
                    className={`overflow-hidden flex justify-between mx-4 py-2 rounded-lg cursor-pointer ${theme === 'dark-theme' ? `${selectedGroup === group.name ? 'bg-gray-800 hover:bg-gray-900' : ''} hover:bg-gray-700 ` : `${selectedGroup === group.name ? 'bg-gray-300 hover:bg-gray-400' : ''}hover:bg-gray-200`}`}
                    key={group._id} >
    ...
    return (
        <div className="flex flex-row text-sm">
            <div
                id="mobile"
                style={{ height: '90vh' }}
                className={`${theme === 'dark-theme' ? 'bg-black text-white' : 'bg-white text-gray-800'} flex flex-col  overflow-y-auto overflow-x-hidden ${isMobile ? `${type ? `${name ? 'hidden' : 'w-full'}` : 'hidden'}` : 'w-1/3'}`} >
