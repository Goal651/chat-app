...
export default function Dashboard({ isMobile }) {
    ...
    const socket = useSocket("http://10.12.75.175:3001");
    ...
    useEffect(() => {
        const fetchUserDetails = async () => {
            ...
            try {
                const response = await fetch(`http://10.12.75.175:3001/getUserProfile`, { headers: { accessToken: `${accessToken}` }, });
        ...
        const fetchInitialData = async () => {
            try {
                const [friendsResponse, groupsResponse] = await Promise.all([
                    fetch("http://10.12.75.175:3001/allFriends", {
                        headers: { accessToken },
                    }),
                    fetch("http://10.12.75.175:3001/allGroups", {
                        headers: { accessToken },
                    }),
                ]);
    ...
    const updateFriends = async () => {
        try {
            const response = await fetch("http://10.12.75.175:3001/allFriends", {
    ...
    const updateGroups = async () => {
        try {
            const response = await fetch("http://10.12.75.175:3001/allGroups", {
                headers: { accessToken: Cookies.get("accessToken") },
            });
            const data = await response.json();
